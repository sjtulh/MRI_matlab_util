...
weights_Sharp = zeros([N, length(Kernel_Sizes)]);
...
% Most exterior layer
phs = phs + DiffMask(:,:,:,end) .* real(ifftn(Phase_unwrap) - SMV(ifftn(Phase_unwrap).*mask_bet, N, voxel_size, Kernel_Sizes(end)/2).*weights_Sharp(:,:,:,end));
