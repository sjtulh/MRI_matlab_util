...
try
    ...
    if flag_parallel
        eval(['parpool(', num2str(n_parallel), ')']);
        n_par = n_parallel;
    else
        n_par = 0;
    end

    parfor (i_echo = 1:necho, n_par)
        iField = zeros([matrix_size, 1, ncoil], 'single');
        for i_coil = 1:ncoil       
        ...
        save_dummy([iField_name, '_grid_echo', num2str(i_echo), '.mat'], 2, 'iField', iField, ...
                                                                            'param_recon', param_recon, '-v7.3')
%         clear iField  
    end 
    delete(gcp('nocreate'))
...
%     save forpar.mat lambda cg_tol cg_max_iter cg_tol_noise
    clear vars_forpar
    vars_forpar.lambda = lambda;
    vars_forpar.cg_tol = cg_tol;
    vars_forpar.cg_max_iter = cg_max_iter;
    vars_forpar.cg_tol_noise = cg_tol_noise;
    vars_forpar.grad = grad;
    vars_forpar.div = div;
    vars_forpar.e_grad = e_grad;
    ...
    parfor (i_echo = 1:necho, n_par)
        ...
        % vars_forpar = load('forpar.mat');
        lambda = vars_forpar.lambda;
        cg_tol = vars_forpar.cg_tol;
        cg_max_iter = vars_forpar.cg_max_iter;
        cg_tol_noise = vars_forpar.cg_tol_noise;
        grad = vars_forpar.grad;
        div = vars_forpar.div;
        e_grad = vars_forpar.e_grad;
        ...
        save_dummy([iField_name, '_SENSE_echo', num2str(i_echo), '.mat'], 13, 'iField', iField,...
                                                                              'res_gE_rel', res_gE_rel,...
                                                                              'iternum', iternum,...
                                                                              'x_hist', x_hist,...
                                                                              'cg_max_iter', cg_max_iter,...
                                                                              'cg_tol', cg_tol,...
                                                                              'cg_tol_noise', cg_tol_noise,...
                                                                              'res_gE_rel_hist', res_gE_rel_hist,...
                                                                              'cost_reg_hist', cost_reg_hist,...
                                                                              'cost_fidelity_hist', cost_fidelity_hist,...
                                                                              'norm_res_gE_fidelity_hist', norm_res_gE_fidelity_hist,...
                                                                              'norm_b_tilde', norm_b_tilde,...
                                                                              'param_recon', param_recon,...
                                                                              '-v7.3');
